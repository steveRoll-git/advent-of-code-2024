...
const findTrailScore = (x: number, y: number, nineMap: Set<string>) => {
  const current = at(x, y);
  if (current == 9 && !nineMap.has(`${x} ${y}`)) {
    nineMap.add(`${x} ${y}`);
    return 1;
  }
  let total = 0;
  for (const d of directions) {
    if (inMap(x + d.x, y + d.y) && at(x + d.x, y + d.y) == current + 1) {
      total += findTrailScore(x + d.x, y + d.y, nineMap);
    }
  }
  return total;
};
...
for (let y = 0; y < mapH; y++) {
  for (let x = 0; x < mapW; x++) {
    if (at(x, y) == 0) {
      total += findTrailScore(x, y, new Set());
    }
  }
}
